...
void SD_LowLevel_Init(void)
{
	...
	GPIO_PinAFConfig(SD_SPI_SCK_GPIO_PORT, SD_SPI_SCK_GPIO_SOURCE, SD_SPI_SCK_GPIO_AF);	//CLK
	GPIO_PinAFConfig(SD_SPI_MISO_GPIO_PORT, SD_SPI_MISO_GPIO_SOURCE, SD_SPI_MISO_GPIO_AF);   //MISO
	GPIO_PinAFConfig(SD_SPI_MOSI_GPIO_PORT, SD_SPI_MOSI_GPIO_SOURCE, SD_SPI_MOSI_GPIO_AF);	//MOSI
...
//  MSD_csd->DeviceSize = (CSD_Tab[7] & 0x3F) << 16;
  /* Byte 7 */
  SD_csd->DeviceSize = (u16)(CSD_Tab[8]) *256;
  /* Byte 8 */
...
{
	unsigned char resp, retry = 0;

// SD Card type check
// Besides the SDHC is address in block, others address in byte
	if(SD_Type != SDHC)
	{		
		if(	  (cmd == SD_CMD_READ_SINGLE_BLOCK)
			...
			||(cmd == SD_CMD_WRITE_MULT_BLOCK)
		   	||(cmd == SD_CMD_SD_ERASE_GRP_START)
			||(cmd == SD_CMD_SD_ERASE_GRP_END)
		  )	// Convert block number to byte address
		{
    		arg = arg << 9;
		}	   
	}
	...
	SD_WriteByte(cmd | 0x40);// send command, first two bits always '01'
	SD_WriteByte(arg >> 24);	// Argument Byte 3, MSB
	SD_WriteByte(arg >> 16);	// Argument Byte 2
	SD_WriteByte(arg >> 8);	// Argument Byte 1
	SD_WriteByte(arg);		// Argument Byte 0, LSB

// Send correct CRC for CMD8 (CRC=0x87) & CMD0 (CRC=0x95)
	if(cmd == 8)
  		SD_WriteByte(0x87);  // For CMD8
	else 
  		SD_WriteByte(0x95);	// For CMD0 

	while((resp = SD_ReadByte()) == 0xFF)	// Wait response
   	{
		if(retry++ > 0xFE)
			break;	//time out error
	}
	
	if(resp == 0x00 && cmd == 58)  // Checking response of CMD58
	{
  		resp = SD_ReadByte() & 0x40;	// First byte of the OCR register (bit 31:24)
  		if(resp == 0x40)
  		{
			SD_Type = SDHC;  // We need it to verify SDHC card
			flag_SDHC = 1;
  		}
 		else
 		{
  		...
  		SD_ReadByte();	// Remaining 3 bytes of the OCR register are ignored here
  		SD_ReadByte();	// One can use these bytes to check power supply limits of SD
  		SD_ReadByte(); 
	}
...
unsigned char SD_MMC_Check(void)
{
	unsigned char r1;
...
// Send CMD55 + ACMD41 repeatedly for checking card is SD V1 or MMC
	do
	{
		do
		{
			r1 = SD_Send_Cmd(55, 0);// CRC dont care
			SD_Dummy_Delay();
...
// Otherwise, send ACMD41 for checking card's version
		r1 = SD_Send_Cmd(41, 0 );// CRC dont care
		SD_Dummy_Delay();
		retry++;
...
// Send CMD 1 for MMC initialization
		do 
		{ 
			r1 = SD_Send_Cmd(SD_CMD_SEND_OP_COND, 0);// CRC dont care
			SD_Dummy_Delay();
			retry++; 
		}while((r1 != 0x00) && (retry < MAX_TIMEOUT)); 

// MMC initialization timeout, return fail
		if(retry == MAX_TIMEOUT)
		{
//			printStr("\n===== Fail!!! =====\n\n");
			return 1;
...
unsigned char SDHC_V2_Check(void)
{
	unsigned char r1, buff[4], test = 0, retry = 0;
	unsigned char i=0;
// Read 4 times to get OCR register
...
// Identify voltage range in 2.7v ~ 3.6v
// This function is reserved in order to support more cards
...
// Send initial command CMD55+ACMD41 repeatedly
	...
	{ 
		do
		{
			r1 = SD_Send_Cmd(55, 6 );// CRC dont care 
			SD_Dummy_Delay();
		}while((r1 !=0x01) && (test < MAX_TIMEOUT));
		if(test == MAX_TIMEOUT)
			return r1;

// Note: SDV2/SDHC must set HCS = 1 in ACMD41 argument
//       indicate host support SDHC or SDXC
		r1 = SD_Send_Cmd(41, 0x40000000 );// CRC dont care
		SD_Dummy_Delay();
		if(retry > MAX_TIMEOUT)
			return r1;
	}while(r1 != 0x00); 
	
// Using CMD58 return response with CCS bit in OCR register
	r1 = SD_Send_Cmd(58, 0 ); // CRC dont care
	SD_Dummy_Delay();

	return SD_Type;
}

void SD_Ver_Check(void)
{
	unsigned char r1;
...
// Send CMD8 to check SD card version
	r1 = SD_Send_Cmd(8, 0x000001AA );//CMD8 crc = 0x87
...
// If CMD8 response is 0x01: SDV2/SDHC
	else if(r1 == 0x01)
	{
		SDHC_V2_Check(); 				
	}
	...
	{
		case SD_V1:
			printf("SD_Type=SD_V1\r\n");
			break;
		case MMC:
			printf("SD_Type=MMC\r\n");
			break;
		case SD_V2:
			printf("SD_Type=SD_V2\r\n");
			break;
		case SDHC:
			printf("SD_Type=SDHC\r\n");
			break;
...
void SPI_Hi_Speed_Config(void)
{
 ...
 /* SPI1 Config */
  ...
  SPI_InitStructure.SPI_CPHA = SPI_CPHA_2Edge;
  SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;
  SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_32;//SPI_BaudRatePrescaler_16 may have background missing issue
  SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;
  SPI_InitStructure.SPI_CRCPolynomial = 7;
  SPI_Init(SD_SPI, &SPI_InitStructure);
