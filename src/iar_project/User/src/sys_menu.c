      ...
      case Sound_Alarm_menu:
        if(key == MSG_UPKeyPress)
        {
          if(sm==ST_MenuChangeValue && Menu[men].Menu_label[label].select==1)
          {
            ...
            Menu[Alarm_Menu].Menu_label[Alarm_VOL_menu].data=10;
            }
          }
        }
        if(key == MSG_DOWNKeyPress)
...
int menu_Label_test=Time_Menu;
void Set_menuLabel(int menu){
    menu_Label_test = menu;
    menu_Label = menu;
}
int Get_menuLabel1(void){
    return menu_Label_test;
}
