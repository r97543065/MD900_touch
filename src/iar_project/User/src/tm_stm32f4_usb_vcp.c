...
int ENG_mod=0,ENG_mod_Pass=0;
void ENG_Return_data(void)
{
  char str[20];
  if(ENG_mod_Pass==1)
  {
    sprintf(str,"SpO2:%3d\r",get_spo2_value());
    TM_USB_VCP_Puts(str);
    
  }
}

void USB_Get_data(void)
{
  
     //   If something arrived at VCP 
      if (TM_USB_VCP_Getc(&USB_DATAc) == TM_USB_VCP_DATA_OK)
      {
        usb_str[0]=(char)USB_DATAc;
        if(usb_str[0]!=0x0D)
        {
          strncat(USB_DATAs,usb_str,1);
        }
      else
      {       
          if(ENG_mod!=1)//工程模式
          {
              if(strcmp("STARTENG",USB_DATAs)==0)
              {
                TM_USB_VCP_Puts("Start test mod\r");
                TM_USB_VCP_Puts("Password:");
                ENG_mod=1;
              }
          }
          else
          {
            ...
            if((strcmp("1452",USB_DATAs)==0)&&(ENG_mod_Pass!=1))
            {
              ENG_mod_Pass=1;
              
              TM_USB_VCP_Puts("Correct\r");
            }
            
            else if(strcmp("ENDENG",USB_DATAs)==0)
            {
              ENG_mod_Pass=0;
              ENG_mod=0;
              TM_USB_VCP_Puts("End ENG Mod\r");
            }
