...
*/                   
	LTDC_InitStruct.LTDC_BackgroundRedValue = 0;//255;            
	LTDC_InitStruct.LTDC_BackgroundGreenValue = 0;//255;          
	LTDC_InitStruct.LTDC_BackgroundBlueValue = 0;//20;  

	/* Configure PLLSAI prescalers for LCD */
	/* Enable Pixel Clock */
	RCC_PLLSAIConfig(Panel_PLLSAI_N, 7, Panel_PLLSAI_R);
	if(Panel_PLLSAI_DivR == 2)
		RCC_LTDCCLKDivConfig(RCC_PLLSAIDivR_Div2);
	else if(Panel_PLLSAI_DivR == 4)
		RCC_LTDCCLKDivConfig(RCC_PLLSAIDivR_Div4);
	else if(Panel_PLLSAI_DivR == 8)
		RCC_LTDCCLKDivConfig(RCC_PLLSAIDivR_Div8);
	else if(Panel_PLLSAI_DivR == 16)
	...
	LTDC_InitStruct.LTDC_HorizontalSync = Panel_HS_Width-1;
	/* Configure vertical synchronization height */
	LTDC_InitStruct.LTDC_VerticalSync = Panel_VS_Height-1;
	/* Configure accumulated horizontal back porch */
	LTDC_InitStruct.LTDC_AccumulatedHBP = Panel_HS_Width+Panel_HBP-1; 
	/* Configure accumulated vertical back porch */
	LTDC_InitStruct.LTDC_AccumulatedVBP = Panel_VS_Height+Panel_VBP-1;  
	/* Configure accumulated active width */  
	LTDC_InitStruct.LTDC_AccumulatedActiveW = Panel_HS_Width+Panel_HBP+
Panel_Active_Width-1;
	/* Configure accumulated active height */
	LTDC_InitStruct.LTDC_AccumulatedActiveH = Panel_VS_Height+Panel_VBP+
Panel_Active_Height-1;
	/* Configure total width */
	LTDC_InitStruct.LTDC_TotalWidth = Panel_HTotal; 
	/* Configure total height */
	LTDC_InitStruct.LTDC_TotalHeigh = Panel_Vtotal;
...
void LCD_Layer1Init(void)
{
  ...
  LTDC_Layer_InitStruct.LTDC_HorizontalStart = Panel_HS_Width+Panel_HBP;
  LTDC_Layer_InitStruct.LTDC_HorizontalStop = (Layer1_Active_Width + Panel_HS_Width+Panel_HBP - 1); 
  LTDC_Layer_InitStruct.LTDC_VerticalStart = Panel_VS_Height + Panel_VBP;
  LTDC_Layer_InitStruct.LTDC_VerticalStop = (Layer1_Active_Height + Panel_VS_Height + Panel_VBP - 1);
...
//  LTDC_Layer_InitStruct.LTDC_PixelFormat = LTDC_Pixelformat_ARGB8888;  
  ...
  */
  LTDC_Layer_InitStruct.LTDC_CFBLineLength = ((Layer1_Active_Width * 2) + 3);
  /* the pitch is the increment from the start of one line of pixels to the 
  start of the next line in bytes, then :
  Pitch = Active high width x number of bytes per pixel */ 
  LTDC_Layer_InitStruct.LTDC_CFBPitch = (Layer1_Active_Width * 2);
  
  /* Configure the number of lines */  
  LTDC_Layer_InitStruct.LTDC_CFBLineNumber = Layer1_Active_Height;
  
  /* Start Address configuration : the LCD Frame buffer is defined on SDRAM */    
  LTDC_Layer_InitStruct.LTDC_CFBStartAdress = Layer1FrameS;
 // LTDC_Layer_InitStruct.LTDC_CFBStartAdress = (uint32_t)&RGB565_800x600;  
...
void LCD_Layer2Init(void)
{
  ...
  Vertical stop   = Vertical start + window height -1  = 4 + 320 -1      */      
  LTDC_Layer_InitStruct.LTDC_HorizontalStart = Panel_HS_Width+Panel_HBP;
  LTDC_Layer_InitStruct.LTDC_HorizontalStop = (Layer2_Active_Width + Panel_HS_Width+Panel_HBP - 1); 
  LTDC_Layer_InitStruct.LTDC_VerticalStart = Panel_VS_Height + Panel_VBP;
  LTDC_Layer_InitStruct.LTDC_VerticalStop = (Layer2_Active_Height + Panel_VS_Height + Panel_VBP - 1);
  ...
  number of bytes per pixel = 2    (pixel_format : RGB565) 
  */
  LTDC_Layer_InitStruct.LTDC_CFBLineLength = ((Layer2_Active_Width * 4) + 3);
  /* the pitch is the increment from the start of one line of pixels to the 
  start of the next line in bytes, then :
  Pitch = Active high width x number of bytes per pixel */ 
  LTDC_Layer_InitStruct.LTDC_CFBPitch = (Layer2_Active_Width * 4);
  
  /* Configure the number of lines */  
  LTDC_Layer_InitStruct.LTDC_CFBLineNumber = Layer2_Active_Height;
  
  /* Start Address configuration : the LCD Frame buffer is defined on SDRAM */    
  LTDC_Layer_InitStruct.LTDC_CFBStartAdress = Layer2FrameS;
...
/**
  ...
  * @retval None
  */
void LCD_DrawFullRect_background(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height)
{
  ...
  Red_Value = (0xF800 & CurrentBackColor) >> 11;
  
  Blue_Value = 0x001F & CurrentBackColor;
  Green_Value = (0x07E0 & CurrentBackColor) >> 5;
...
void LCD_Clear(uint16_t Color)
{
  ...
  BackColorinit(); //≠´∏mbackcolr πw®æº∆≠»©≥¶‚¶≥∞›√D
}
...
uint16_t LCD_BackColor_get(void)
{
  return CurrentBackColor ;
}
/**
...
void LCD_DrawLine2(uint16_t Xpos, uint16_t Ypos, uint16_t Length, uint8_t Direction,    uint16_t Color)
{
  DMA2D_InitTypeDef      DMA2D_InitStruct;
  ...
  // change_color(Old_BackColor,Old_TextColor);
}

void LCD_DrawBox(uint16_t Xpos, uint16_t Ypos, uint16_t Long, uint16_t Width,uint16_t Color)
{
  change_color(Color,LCD_BackColor_get()); 
  LCD_DrawLine(Xpos,Ypos,Long, LCD_DIR_HORIZONTAL);
  LCD_DrawLine(Xpos,Ypos+Width,Long, LCD_DIR_HORIZONTAL);
  LCD_DrawLine(Xpos,Ypos,Width, LCD_DIR_VERTICAL);
  LCD_DrawLine(Xpos+Long,Ypos,Width+1, LCD_DIR_VERTICAL);
  change_color(LCD_COLOR_WHITE,LCD_BackColor_get());
}
/**
...
/*
 ...
 +                       LCD pins assignment                               +
 +------------------------+-----------------------+----------------------------+
 |  LCD_TFT R2 <-> PC.10//PH8   |  LCD_TFT G2 <-> PA.06 |  LCD_TFT B2 <-> PD.06      |
 |  LCD_TFT R3 <-> PB.00  |  LCD_TFT G3 <-> PG.10 |  LCD_TFT B3 <-> PG.11      |
 |  LCD_TFT R4 <-> PA.11//PH10  |  LCD_TFT G4 <-> PB.10 |  LCD_TFT B4 <-> PG.12//PI4      |
 |  LCD_TFT R5 <-> PA.12//PH11  |  LCD_TFT G5 <-> PB.11 |  LCD_TFT B5 <-> PA.03//PI5      |
 |  LCD_TFT R6 <-> PB.01//PH12  |  LCD_TFT G6 <-> PC.07 |  LCD_TFT B6 <-> PB.08//PI6      |
 |  LCD_TFT R7 <-> PG.06        |  LCD_TFT G7 <-> PD.03 |  LCD_TFT B7 <-> PB.09//PI7      |
 -------------------------------------------------------------------------------
          |  LCD_TFT HSYNC <-> PC.06  | LCDTFT VSYNC <->  PA.04//PI9 |
...
*/
 ...
 /* GPIOA configuration */
 // GPIO_PinAFConfig(GPIOA, GPIO_PinSource3, GPIO_AF_LTDC);//PI5
  //GPIO_PinAFConfig(GPIOA, GPIO_PinSource4, GPIO_AF_LTDC);
  GPIO_PinAFConfig(GPIOA, GPIO_PinSource6, GPIO_AF_LTDC);
//  GPIO_PinAFConfig(GPIOA, GPIO_PinSource11, GPIO_AF_LTDC);PH10
//  GPIO_PinAFConfig(GPIOA, GPIO_PinSource12, GPIO_AF_LTDC);PH11
 ...
 // GPIO_PinAFConfig(GPIOB, GPIO_PinSource1, 0x09);//PH12
 // GPIO_PinAFConfig(GPIOB, GPIO_PinSource8, GPIO_AF_LTDC);//PI6
 // GPIO_PinAFConfig(GPIOB, GPIO_PinSource9, GPIO_AF_LTDC);//PI7
 ...
 /* GPIOC configuration */
  ...
  //GPIO_PinAFConfig(GPIOC, GPIO_PinSource10, GPIO_AF_LTDC);//PH8
 ...
 /* GPIOG configuration */  
  ...
  /* GPIOH configuration */
  GPIO_PinAFConfig(GPIOH, GPIO_PinSource8, GPIO_AF_LTDC);//PH8
  GPIO_PinAFConfig(GPIOH, GPIO_PinSource10, GPIO_AF_LTDC);//PH10
  GPIO_PinAFConfig(GPIOH, GPIO_PinSource11, GPIO_AF_LTDC);//PH11
  GPIO_PinAFConfig(GPIOH, GPIO_PinSource12, GPIO_AF_LTDC);//PH12
 ...
 // RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOH, ENABLE);  
  ...
  GPIO_InitStruct.GPIO_Pin = GPIO_Pin_13 |GPIO_Pin_14|GPIO_Pin_15; // ≥]©w≤ƒ  4 5 7∏}
  GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_InitStruct.GPIO_OType = GPIO_OType_PP;
  GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_UP;
  GPIO_InitStruct.GPIO_Mode = GPIO_Mode_OUT; // ≥]∏m¨∞¬–•Œ
  GPIO_Init(GPIOH, &GPIO_InitStruct); // ÆM•Œ•H§W GPIO ≥]∏m°A®√™Ï©l§∆ GPIOD
  //GPIO_SetBits(GPIOH,GPIO_Pin_15);//20170523Â∑≤Á∂ìËÆäÊàêNURSECALLÁöÑÂäüËÉΩ ÊâÄ‰ª•ÂèñÊ∂à
  GPIO_SetBits(GPIOH,GPIO_Pin_14);
  GPIO_ResetBits(GPIOH,GPIO_Pin_13);  
  /* GPIOI configuration */
  GPIO_PinAFConfig(GPIOI, GPIO_PinSource9, GPIO_AF_LTDC);
  GPIO_PinAFConfig(GPIOI, GPIO_PinSource4, GPIO_AF_LTDC);//PI4
  GPIO_PinAFConfig(GPIOI, GPIO_PinSource5, GPIO_AF_LTDC);//PI5
  GPIO_PinAFConfig(GPIOI, GPIO_PinSource6, GPIO_AF_LTDC);//PI6
  GPIO_PinAFConfig(GPIOI, GPIO_PinSource7, GPIO_AF_LTDC);//PI7
  GPIO_InitStruct.GPIO_Pin = GPIO_Pin_4 |GPIO_Pin_5 |GPIO_Pin_6 |GPIO_Pin_7 |GPIO_Pin_9  ;
...
void PutPixel_2(int16_t x, int16_t y,    uint16_t Color)
{ 
  if(x < 0 || x > Panel_Active_Width-1 || y < 0 || y > Panel_Active_Height-1)
  {
    
  }
  else
  {
    LCD_DrawLine2(x, y, 1, LCD_DIR_HORIZONTAL,Color);
  }
}
#ifndef USE_Delay
/**
