...
int F_ADC_buffer[7]= {3000};
int F_ADC2_buffer[7]= {1};
int current_data=0;
int power_test;
/* Private functions ---------------------------------------------------------*/
...
/**
  ...
  */
/*******Get_the_Power_dat**********/
int Start_power_check(void)
{
  return ADC_GetConversionValue(ADC3);
}
int  Get_Power_dat (void)
{


    int Power_ADC;
    wchar_t strx[30];
    F_ADC_buffer[1]=ADC_GetConversionValue(ADC3);
    F_ADC_buffer[2]=ADC_GetConversionValue(ADC3);
    F_ADC_buffer[3]=ADC_GetConversionValue(ADC3);
    F_ADC_buffer[4]=ADC_GetConversionValue(ADC3);
    F_ADC_buffer[5]=ADC_GetConversionValue(ADC3);
   // F_ADC2_buffer[1]=ADC_GetConversionValue(ADC2);
    for (int i=0; i<5; i++)
    {
        for(int j=i+1; j<5; j++)
        {
            if (F_ADC_buffer[i]>F_ADC_buffer[j])
            {

                Power_ADC=F_ADC_buffer[j];
                F_ADC_buffer[j]=F_ADC_buffer[i];
                F_ADC_buffer[j]=Power_ADC;
    ...
    if (F_ADC_buffer[6]==0)
    {
        F_ADC_buffer[6]=F_ADC_buffer[3];
    }
    Power_ADC=(F_ADC_buffer[2]+F_ADC_buffer[3]+F_ADC_buffer[4]+F_ADC_buffer[6]*17)/20;
    F_ADC_buffer[6]=Power_ADC;
  //  change_color(LCD_COLOR_WHITE,LCD_COLOR_BLACK);
    Get_Power_Current();
   // power_test = ((int)(((float)Power_ADC/(float)current_data)*10000))/100;
             /********估算 0.1V =26********/
    /**20180125 推算0.1A =>200讀值*/
    //swprintf(strx,20,L"ADC: %6d:%d:  ",Power_ADC,current_data);
    //show_str2(200,50,strx);
   // Get_Power_Current();
    
    //2970 = 11.5v, 2840 = 11v, 2710 = 10.5v, 2580 = 10v, 2450 =9.5v
    //2695 = 11.5v, 2560 = 11v, 2450 = 10.5v, 2310 = 10v, 2170 =9.5v   //20200924
    //20170525因為電池座元件會篩檢電池的電壓,大約差-0.5v,所以閥值都要下調0.5v
    //return DC_in;
    
         if (Power_ADC>=3000)//3090=12V   
         {
           //Set_INOP_ON(AC_IN_INOP);
           Set_Alarm_OFF(Low_Battery_alarm);
           //Set_INOP_OFF(BATTERY_50_POWER);
           //Set_INOP_OFF(BATTERY_25_POWER);
           //Set_INOP_OFF(Low_Battery_INOP);
           //Set_INOP_OFF(BATTERY_F_POWER);
           //DC_LED_HIGH();//2021
           LBATTERY_LED_HIGH();
           //BATTERY_LED_HIGH();//2021

           return DC_in;
         }
         else if (Power_ADC>=2450&&Power_ADC<3000)//11.5 - 0.5      //2700-->2450
         {
           //Set_INOP_ON(BATTERY_F_POWER);
           Set_Alarm_OFF(Low_Battery_alarm);
           //Set_INOP_OFF(BATTERY_50_POWER);
           //Set_INOP_OFF(BATTERY_25_POWER);
           //Set_INOP_OFF(Low_Battery_INOP);
           //Set_INOP_OFF(AC_IN_INOP);
           //DC_LED_LOW();//2021
           LBATTERY_LED_LOW();
           //BATTERY_LED_HIGH();//2021
         ...
         else if (Power_ADC>=2400&&Power_ADC<2450)//11.0 - 0.5     //2650-->2400    2700-->2450
         {
           Set_Alarm_OFF(Low_Battery_alarm);
           //Set_INOP_ON(BATTERY_50_POWER);
           //Set_INOP_OFF(BATTERY_F_POWER);
           //Set_INOP_OFF(BATTERY_25_POWER);
           //Set_INOP_OFF(Low_Battery_INOP);
           //Set_INOP_OFF(AC_IN_INOP);
         ...
         else if (Power_ADC>=2370&&Power_ADC<2400)//10.5 - 0.5    //2620-->2370   2650-->2400
         {
           //Set_INOP_ON(BATTERY_25_POWER);
           //Set_INOP_OFF(BATTERY_F_POWER);
           //Set_INOP_OFF(BATTERY_50_POWER);
           //Set_INOP_OFF(Low_Battery_INOP);
           //Set_INOP_OFF(AC_IN_INOP);
         ...
         else if (Power_ADC>=2350&&Power_ADC<2370)//10 - 0.5  //2600-->2350  2620-->2370
         {
           //Set_INOP_ON(Low_Battery_INOP);
           Set_Alarm_ON(Low_Battery_alarm);
           //Set_INOP_OFF(BATTERY_F_POWER);
           //Set_INOP_OFF(BATTERY_50_POWER);
           //Set_INOP_OFF(BATTERY_25_POWER);
           //Set_INOP_OFF(AC_IN_INOP);
         ...
         else if (Power_ADC<2350)//關機  //2600-->2350
         {
           //Set_INOP_ON(Low_Battery_INOP);
           //Set_INOP_OFF(BATTERY_F_POWER);
           //Set_INOP_OFF(BATTERY_50_POWER);
           ...
           //BATTERY_LED_LOW();//2021
            ...
            //sys_mem_write();
            swprintf(strx,40,L"WWW.comdek.com");
            show_str(310,290,strx);
...
int  Get_Power_Current (void)
{
    ...
    //wchar_t strx[20];
    F_ADC2_buffer[1]=ADC_GetConversionValue(ADC2);
    F_ADC2_buffer[2]=ADC_GetConversionValue(ADC2);
    F_ADC2_buffer[3]=ADC_GetConversionValue(ADC2);
    F_ADC2_buffer[4]=ADC_GetConversionValue(ADC2);
    F_ADC2_buffer[5]=ADC_GetConversionValue(ADC2);
    for (int i=0; i<5; i++)
    {
        for(int j=i+1; j<5; j++)
        {
            if (F_ADC2_buffer[i]>F_ADC2_buffer[j])
            {

                Power_ADC=F_ADC2_buffer[j];
                F_ADC2_buffer[j]=F_ADC2_buffer[i];
                F_ADC2_buffer[j]=Power_ADC;

            }
        }
    }
    Power_ADC=(F_ADC2_buffer[2]+F_ADC2_buffer[3]+F_ADC2_buffer[4])/3;
    current_data=Power_ADC;
    change_color(LCD_COLOR_WHITE,LCD_COLOR_BLACK);
   ///    swprintf(strx,20,L"ADC: %6d",current_data,F_ADC2_buffer[2]);
  //   show_str2(200,90,strx);
    return current_data;
